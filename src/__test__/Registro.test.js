...
describe('Registro Component', () => {
    ...
    const renderRegistro = (ultimoEventoMock) => {
        return render(
            <DatosJugadorContext.Provider value={{ datosJugador: datosJugadorMock }}>
                <DatosPartidaContext.Provider value={{ datosPartida: datosPartidaMock }}>
    ...
    it('debería mostrar el mensaje de unión a la partida cuando el evento es GET_PLAYER_MATCH_INFO', async () => {
        ...
        await waitFor(() => expect(screen.getByText("Te has unido a la partida, tu orden de turno es: 1")).toBeInTheDocument());
    });
    ...
    it('debería mostrar el mensaje cuando el jugador recibe una carta de figura', async () => {
        ...
        await waitFor(() => expect(screen.getByText('El jugador "Player1" ha recibido la carta de figura "MockedFigura1".')).toBeInTheDocument());
    });
    ...
    it('debería mostrar el mensaje cuando el jugador termina su turno', async () => {
        ...
        await waitFor(() =>
            expect(screen.getByText('El jugador "Player1" ha terminado su turno.')).toBeInTheDocument()
        );
        await waitFor(() =>    
    ...
    it('debería mostrar el mensaje de deshacer movimiento', async () => {
        ...
        await waitFor(() =>
            expect(screen.getByText('El jugador "Player1" ha deshecho un movimiento.')).toBeInTheDocument()
        );
    });
    ...
    it('no debería añadir eventos si `ultimoEvento` es null', () => {
        ...
        expect(screen.queryByText('Te has unido a la partida')).not.toBeInTheDocument();
    });
});