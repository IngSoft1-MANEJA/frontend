...
describe('ServicioMovimiento', () => {
  ...
  describe('calcularMovimientos', () => {
    test('debería calcular movimientos posibles para la carta "Diagonal"', () => {
      const movimientos = ServicioMovimiento.calcularMovimientos(2, 2, 'Diagonal');
    ...
    test('debería calcular movimientos posibles para la carta "Line Between"', () => {
      const movimientos = ServicioMovimiento.calcularMovimientos(2, 2, 'Line Between');
      expect(movimientos).toEqual([
        [0, 2], [4, 2], [2, 0], [2, 4]
      ]);
    });
    ...
    test('debería calcular movimientos posibles para la carta "Inverse Diagonal"', () => {
      const movimientos = ServicioMovimiento.calcularMovimientos(2, 2, 'Inverse Diagonal');
      expect(movimientos).toEqual([
        [1, 1], [1, 3], [3, 1], [3, 3]
      ]);
    });
    ...
    test('debería calcular movimientos posibles para la carta "Inverse L"', () => {
      const movimientos = ServicioMovimiento.calcularMovimientos(2, 2, 'Inverse L');
      expect(movimientos).toEqual([
        [0, 3], [4, 1], [1, 0], [3, 4]
      ]);
    });
    ...
    test('debería calcular movimientos posibles para la carta "Line Border"', () => {
      const movimientos = ServicioMovimiento.calcularMovimientos(2, 2, 'Line Border');
      expect(movimientos).toEqual([
        [0, 2], [2, 0], [2, 5], [5, 2]
      ]);
    });
    ...
    test('debería devolver un array vacío si se pasa una carta desconocida', () => {
      const movimientos = ServicioMovimiento.calcularMovimientos(2, 2, 'Carta Desconocida');
      expect(movimientos).toEqual([]);
    });
  });
  ...
  describe('swapFichas', () => {
    ...
    test('debería intercambiar dos fichas correctamente', () => {
      ...
      ServicioMovimiento.swapFichas(fichasSeleccionadas, tiles, setTilesMock, setUsarMovimientoMock);
