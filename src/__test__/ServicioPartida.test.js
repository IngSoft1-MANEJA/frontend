...
const server = setupServer(
  ...
  http.delete(`${BACKEND_URL}/matches/:idPartida/partial-move/:idJugador`, () => {
    return HttpResponse.json({ undone: true });
  }),
  http.post(`${BACKEND_URL}/matches/:idPartida/player/:idJugador/use-figure`, () => {
    const figure_id = 123;
...
describe("ServicioPartida", () => {
  ...
  it("debería crear una partida correctamente", async () => {
    const respuesta = await ServicioPartida.crearPartida("Partida 1", "Jugador 1", 4);
  ...
  it("debería lanzar un error si falla al crear una partida", async () => {
    ...
    await expect(ServicioPartida.crearPartida("Sala 1", "Jugador 1", 4)).rejects.toThrow(
      "Error al crear partida - estado: 500"
    );
  });
  ...
  it("debería lanzar un error si falla al iniciar una partida", async () => {
    server.use(
      http.patch(`${BACKEND_URL}/matches/:id/start/:playerId`, (req, res, ctx) => {
        return res(ctx.status(500));
      })
    );
  ...
  it("debería lanzar un error si falla al abandonar una partida", async () => {
    server.use(
      http.delete(`${BACKEND_URL}/matches/:id/left/:playerId`, (req, res, ctx) => {
        return res(ctx.status(500));
      })
    );
  ...
  it("debería lanzar un error si falla al obtener la información del jugador", async () => {
    ...
    await expect(ServicioPartida.obtenerInfoPartidaParaJugador(1, 2)).rejects.toThrow(
      "Error al obtener info de partida - estado: 500"
    );
  });
  ...
  it("debería lanzar un error si falla al deshacer el movimiento parcial", async () => {
    ...
    await expect(ServicioPartida.deshacerMovimientoParcial(1, 2)).rejects.toThrow(
      "Error al deshacer movimiento parcial - estado: 500"
    );
  });

  it("debería completar una figura correctamente", async () => {
    const response = await ServicioPartida.completarFicha(1, 2, 123, [[1, 1], [2, 2]]);
  ...
  it("debería lanzar un error si falla al completar una figura", async () => {
    ...
    await expect(ServicioPartida.completarFicha(1, 2, 123, [[1, 1], [2, 2]])).rejects.toThrow(
      "Error al validar movimiento - estado: 500"
    );
  });
});
